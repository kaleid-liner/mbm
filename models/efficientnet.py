...
class EfficientNet(nn.Module):
    def __init__(
        ...
        modified_settings: List[List[MBConvConfig]] = None,
        **kwargs: Any,
    ) -> None:
        """
        ...
        if modified_settings is not None:
            first_setting, second_setting = modified_settings
        else:
            first_setting = second_setting = inverted_residual_setting

        # building first layer
        firstconv_output_channels = inverted_residual_setting[0].input_channels
        self.firstconv = ConvBNActivation(
                3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.SiLU
        )

        # building inverted residual blocks
        first_stage_blocks = sum(cnf.num_layers for cnf in first_setting)
        first_conv1x1s: List[nn.Module] = []
        stage_block_id = 0
        first_stages: List[nn.Module] = []
        for cnf, first_cnf in zip(inverted_residual_setting, first_setting):
            stage: List[nn.Module] = []
            for _ in range(first_cnf.num_layers):
                # copy to avoid modifications. shallow copy is enough
                block_cnf = copy.copy(first_cnf)
                ...
                sd_prob = stochastic_depth_prob * float(stage_block_id) / first_stage_blocks

                stage.append(block(block_cnf, sd_prob, norm_layer))
                stage_block_id += 1
            
            first_conv1x1s.append(nn.Sequential(
                nn.Conv2d(block_cnf.out_channels, cnf.out_channels // 2, 1),
                nn.BatchNorm2d(cnf.out_channels // 2),
            ))

            first_stages.append(nn.Sequential(*stage))

        second_stage_blocks = sum(cnf.num_layers for cnf in second_setting)
        second_conv1x1s: List[nn.Module] = []
        stage_block_id = 0
        second_stages: List[nn.Module] = []
        for cnf, second_cnf in zip(inverted_residual_setting, second_setting):
            stage: List[nn.Module] = []
            for _ in range(second_cnf.num_layers):
                # copy to avoid modifications. shallow copy is enough
                block_cnf = copy.copy(second_cnf)

                # overwrite info if not the second conv in the stage
                if stage:
                    block_cnf.input_channels = block_cnf.out_channels
                    block_cnf.stride = 1

                # adjust stochastic depth probability based on the depth of the stage block
                sd_prob = stochastic_depth_prob * float(stage_block_id) / second_stage_blocks
            ...
            second_stages.append(nn.Sequential(*stage))

        # building last several layers
        lastconv_input_channels = inverted_residual_setting[-1].out_channels
        lastconv_output_channels = 4 * lastconv_input_channels
        self.lastconv = ConvBNActivation(
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        for first_stage, first_conv1x1, second_stage, second_conv1x1 in zip(
            self.first_stages,
            self.first_conv1x1s,
            self.second_stages,
            self.second_conv1x1s
        ):
            x1 = first_stage(x)
            x1 = first_conv1x1(x1)
            x2 = second_stage(x)
...
def efficientnet_b3(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> EfficientNet:
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
        MBConvConfig(expand_ratio=1, kernel=3, stride=1, input_channels=40, out_channels=24, num_layers=2)
        MBConvConfig(expand_ratio=6, kernel=3, stride=2, input_channels=24, out_channels=32, num_layers=3)
        MBConvConfig(expand_ratio=6, kernel=5, stride=2, input_channels=32, out_channels=48, num_layers=3)
        MBConvConfig(expand_ratio=6, kernel=3, stride=2, input_channels=48, out_channels=96, num_layers=5)
        MBConvConfig(expand_ratio=6, kernel=5, stride=1, input_channels=96, out_channels=136, num_layers=5)
        MBConvConfig(expand_ratio=6, kernel=5, stride=2, input_channels=136, out_channels=232, num_layers=6)
        MBConvConfig(expand_ratio=6, kernel=3, stride=1, input_channels=232, out_channels=384, num_layers=2)
    """
